#ifndef BDX_DEMO_MORSE_SPHERE_HPP
#define BDX_DEMO_MORSE_SPHERE_HPP

#include "util.hpp"
#include "LennardJonesSphere.hpp"

struct MorseSphere : LennardJonesSphere
{
	double _epsilonMP_;
	double _sigmaMP_;
	double _rangeMP_;
	double _equilibriumDistanceMP_;
	MorseSphere(
			long const kind,
			long const group,
			long const feat,
	...
	void MP(struct Particle const * const Particle,
			double const L,
			double const W,
			double const H,
			double const R);
